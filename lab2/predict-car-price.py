...
class CarPrice:
    ...
    def convertToLog(self, df_train, df_val, df_test):
...
def main():
    ...
    df_train, df_val, df_test = obj.validate()
    df_test_orig = df_test.copy()

    y_train_orig, y_val_orig, y_test_orig, y_train, y_val, y_test = obj.convertToLog(
        df_train, df_val, df_test)
    X_train = obj.prepare_X(df_train)
    w_0, w = obj.linear_regression(X_train, y_train)

    X_val = obj.prepare_X(df_val)
    y_pred = w_0 + X_val.dot(w)
    # print('validation:', obj.rmse(y_val, y_pred))
